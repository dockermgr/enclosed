...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202409121701-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Thursday, Sep 12, 2024 17:01 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for enclosed
...
CONTAINER_WEB_SERVER_VHOSTS="notes.all"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
TASK_DELETE_EXPIRED_NOTES_CRON="*/60 * * * *"
TASK_DELETE_EXPIRED_NOTES_RUN_ON_STARTUP=true
STORAGE_DRIVER_FS_LITE_PATH=/data/enclosed
STORAGE_DRIVER_CLOUDFLARE_KV_BINDING=notes

EOF
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/enclosed.script.sh
...
if [ -n "$ENCLOSED_USERNAME" ]; then
  CONTAINER_USER_NAME="$ENCLOSED_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${ENCLOSED_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$ENCLOSED_PASSWORD" ]; then
  CONTAINER_USER_PASS="$ENCLOSED_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${ENCLOSED_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
